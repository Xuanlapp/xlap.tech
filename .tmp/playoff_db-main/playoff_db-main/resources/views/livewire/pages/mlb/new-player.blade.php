<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-5 py-5">
    ...
    <div class="flex bg-white shadow-sm rounded-md p-3">
        <div class="flex-1 w-full space-y-3">
            ...
            <input class="border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block md:w-1/3 w-full p-2.5" type="text" wire:model="search_player" placeholder="Type player name here">
        </div>
    </div>
    {{-- Filter end --}}
                        ...
                        <div class="px-3">
                            <x-dropdown align="right" width="48">
                                ...
                                <x-slot name="content">
                                    ...
                                    <x-dropdown-link wire:click='putOnHold({{$player->id}})'>
                                        ‼️ Put On Hold
                                    </x-dropdown-link>
                                    <a class="block px-4 py-2 text-lg leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition" wire:click='$emit("openModal", "modals.mlb.confirm-dialog", {{json_encode(['message'=> 'Are you sure want to remove '. $player->panini_full_name.'?', 'player_id' => $player->id])}})'>❌ Remove Player</a>
                                </x-slot>
                            </x-dropdown>
                        </div>
                    @endrole
