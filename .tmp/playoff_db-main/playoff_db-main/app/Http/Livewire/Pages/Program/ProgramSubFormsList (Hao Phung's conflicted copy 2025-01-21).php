...
//            ->orderBy('id')->get();
    ...
    public function loadData()
    {
        ...
        if ($this->search !== '') {
            return $query->where('insert_name', 'like', '%' . $this->search . '%')->get();
        }
        if ($this->sortOption === 'insert_name') {
            return $query->orderBy('insert_name', $this->sortOrder)->orderBy('form')->get();
        } else {
            return $query->orderBy('form', $this->sortOrder)->get();
        }
