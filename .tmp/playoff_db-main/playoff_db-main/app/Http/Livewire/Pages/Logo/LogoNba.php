...
class LogoNba extends Component
{
    ...
    {
        return view('livewire.pages.logo.Nba.logo-nba', [
            'logos' => $this->loadData(),
    ...
    {
        $query = logo_nba_versions::with('nba_team');
        if ($this->selected_team !== '') {
            $query = $query->whereHas('nba_team', function ($query) {
                $query->where('kind', $this->selected_team); // Lọc theo 'kind'
            });
        }
        if ($this->search_player !== '') {
            $query = $query->whereHas('nba_team', function ($query) {
                $query->where('team_name', 'like', '%' . $this->search_player . '%');
            });
        }
