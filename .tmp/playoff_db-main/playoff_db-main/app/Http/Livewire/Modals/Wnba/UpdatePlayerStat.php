...
class UpdatePlayerStat extends ModalComponent
{
    ...
    {
        return view('livewire.modals.wnba.update-player-stat');
    }
    ...
    public function boot(WnbaFetchDataService $dataHandler)
    {
    ...
    protected function resumeUpdates()
    {
        ...
        if ($playerId) {
            ...
            try {
                // 更新操作
                $player = Panini_wnba_player::where('id', $playerId)->first();
...
//        try {
//            $player = Panini_wnba_player::where('id', $playerId)->first();
//            if ($player && $this->dataHandler->saveData($player, 'update')) {
//                $this->removePlayerIdFromLog($playerId);
//            } else {
//                throw new Exception('Data is EMPTY');
//            }
//        } catch (Exception $e) {
//            Log::channel('mylog')->error("Update player #{$playerId} Error: {$e->getMessage()}");
//        }
//
//        sleep(1);
//        $this->resumeUpdates();
