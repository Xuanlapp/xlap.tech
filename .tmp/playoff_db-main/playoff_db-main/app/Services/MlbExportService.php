...
class MlbExportService
{
    ...
    private function csvBody($row)
    {
        ...
        */
        ...
        $remove_items_array = ['MlbId', 'status', 'statTitle', 'statOneYear', 'career'];
    ...
    {
        $output_str = '';
        $player = panini_mlb_player::where('panini_id', $player_id)->first();
        // dd($player_id);
        ...
        foreach ($player->show_stat_with_quantity(1) as $year) {
            $output_str .= implode("\t", $year->show_stat()) . " ";
        }
    ...
    private function csvTitle($fullShowingData, $selectedItems)
    {
        $selected_players = collect($fullShowingData)->whereIn('Card #', $selectedItems);
        /*    
    ...
    public function prepShowingData($filePath, $sport)
    {
        ...
        if ($sportForThisExport == $sport) {
            ...
            foreach ($data as $key => $row) {
                ...
                $player = panini_mlb_player::where('panini_id', $row['PlayerID'])->get()->first();
    ...
    {
        // var_dump($player);
        $data['MlbId'] = $player->mlb_player_id;
    ...
    /**
     * Create an Email report specific for MLB
     *
     * @param  mixed $data
     ...
     * @return void
     */
    public function generateEmailMlb($data, $fullShowingData, $selectedItems)
    {
        $selected_players = collect($fullShowingData)->whereIn('Card #', $selectedItems);
        $newLine = "%0D%0A";
