<section class="min-h-[calc(100vh-4rem)] bg-[#f4f6fb] px-3 py-6 text-slate-950 sm:px-5 lg:px-8">
    <div class="mx-auto flex w-full max-w-[1450px] flex-col gap-5">
        ...
        @if ($showManualFbaModal)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4"><div class="flex max-h-[92vh] w-full max-w-5xl flex-col rounded-2xl bg-white p-6 shadow-2xl"><div class="flex items-start justify-between"><div><h2 class="text-lg font-black">Len don FBA bang tay</h2><p class="mt-1 text-sm text-slate-500">Nhap Size (in) va Pack de tim Product ID FBA, sau do export cac dong hop le.</p></div><button type="button" wire:click="closeManualFbaModal" class="text-2xl text-slate-400">&times;</button></div><div class="mt-4 grid gap-3 sm:grid-cols-3"><div><label class="mb-1 block text-xs font-bold text-slate-600">Size (in)</label><input type="number" min="1" step="0.1" wire:model.live.debounce.400ms="manualFbaSize" placeholder="4" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"></div><div><label class="mb-1 block text-xs font-bold text-slate-600">Pack</label><input type="number" min="1" wire:model.live.debounce.400ms="manualFbaPack" placeholder="3" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"></div><div><label class="mb-1 block text-xs font-bold text-slate-600">Product</label><div class="max-h-10 overflow-auto rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">{{ collect($manualFbaPreviewRows)->pluck('product_name')->filter()->unique()->implode(' | ') ?: '-' }}</div></div></div><div class="mt-4 max-h-[480px] overflow-auto rounded-lg border"><table class="w-full min-w-[700px] text-left text-sm"><thead class="sticky top-0 bg-emerald-500 text-white"><tr><th class="px-3 py-2">SKU</th><th class="px-3 py-2">Product ID</th><th class="px-3 py-2">Quantity</th><th class="px-3 py-2">Link Design</th><th class="px-3 py-2">Status</th></tr></thead><tbody>@foreach ($manualFbaPreviewRows as $row)<tr class="{{ $row['error'] !== '' ? 'bg-rose-50' : '' }}"><td class="px-3 py-2 font-bold">{{ $row['sku'] }}</td><td class="px-3 py-2 font-bold">{{ $row['product_id'] ?: '-' }}</td><td class="px-3 py-2">{{ $row['quantity'] }}</td><td class="max-w-72 truncate px-3 py-2"><a href="{{ $row['link_design'] }}" target="_blank" class="text-blue-600">{{ $row['link_design'] }}</a></td><td class="px-3 py-2 {{ $row['error'] !== '' ? 'text-rose-700' : 'text-emerald-700' }}">{{ $row['error'] ?: 'Ready' }}</td></tr>@endforeach</tbody></table></div><div class="mt-5 flex justify-end gap-2"><button type="button" wire:click="closeManualFbaModal" class="rounded-lg border px-4 py-2 text-sm font-bold">Close</button>@if (collect($manualFbaPreviewRows)->contains(fn ($row) => $row['error'] === ''))<button type="button" wire:click="exportManualFbaOrders" class="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white">OK & Tai Excel</button>@endif</div></div></div>
